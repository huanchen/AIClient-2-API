...
const translations = {
    'zh-CN': {
        ...
        'config.sessionAffinity.title': '会话粘连与负载均衡',
        'config.sessionAffinity.enabled': '启用会话粘连',
        'config.sessionAffinity.enabledNote': '同一会话（多轮对话）尽量绑定到同一个节点，提高缓存命中率和稳定性',
        'config.sessionAffinity.defaultWeakTtl': '弱会话 TTL（毫秒）',
        'config.sessionAffinity.defaultWeakTtlNote': '基于 API Key + IP + UA 识别的弱会话默认过期时间，默认 1800000 = 30 分钟',
        'config.sessionAffinity.default5xxCoolDown': '5xx 冷却时间（毫秒）',
        'config.sessionAffinity.default5xxCoolDownNote': '服务端错误后节点冷却时间，默认 60000 = 1 分钟',
        'config.sessionAffinity.max429CoolDown': '429 最大冷却时间（毫秒）',
        'config.sessionAffinity.max429CoolDownNote': '限流错误指数退避最大冷却时间，默认 3600000 = 1 小时',
        'config.sessionAffinity.maxSessions': '最大缓存会话数',
        'config.sessionAffinity.maxSessionsNote': '内存中缓存的最大会话数量，超出后按 LRU 淘汰',
        'config.sessionAffinity.virtualNodes': '一致性哈希虚拟节点数',
        'config.sessionAffinity.virtualNodesNote': '每个物理节点对应多少虚拟节点，越大分布越均匀，默认 150',
        ...
        'config.routing.editorLabel': '客户端模型路由表 (JSON)',
        'config.routing.editorPlaceholder': '治理 Claude / Codex 客户端模型名到上游真实模型的映射表',
        'config.routing.editorNote': '这里只保存你的覆盖项；下方关系表展示当前生效后的完整映射，包含 Claude / OpenAI 协议别名、Claude/Codex 到 Gemini/Claude/OpenAI/Grok 的上游路由，以及 `-1M` 这类客户端别名。',
        'config.routing.previewTitle': '客户端模型关系表',
        'config.routing.previewNote': '上面的 JSON 文本框用于治理覆盖项；下面两张表用来核对当前生效的“客户端 -> Provider -> 上游模型”结果。',
        'config.routing.aliasTableTitle': '协议别名归一化',
        'config.routing.aliasTableDescription': '整理 Claude 协议与 OpenAI/Codex 协议下游可识别的模型名、1M/-1M 客户端别名和真实模型名归一化结果。',
        'config.routing.providerTableTitle': 'Provider 上游映射',
        'config.routing.providerTableDescription': '展示 Claude / Codex 客户端在不同上游 Provider 家族下最终会请求什么模型。',
        'config.routing.columns.client': '下游客户端',
        'config.routing.columns.protocol': '协议',
        'config.routing.columns.canonicalModel': '规范化模型',
        'config.routing.columns.aliases': '别名 / 输入方式',
        'config.routing.columns.requestModel': '请求模型',
        'config.routing.columns.provider': 'Provider 目标',
        'config.routing.columns.upstreamModel': '上游模型',
        'config.routing.columns.note': '说明',
        'config.routing.empty': '暂无可展示的路由规则。',
        'config.routing.invalidJson': '客户端模型路由表格式无效，请输入有效的 JSON 对象',
        'config.routing.previewInvalid': '当前 JSON 无法解析，关系表暂时无法预览。请修复为合法对象后再保存。',
        'config.routing.client.claude': 'Claude 客户端',
        'config.routing.client.codex': 'Codex 客户端',
        'config.routing.protocol.claude': 'Claude 协议',
        'config.routing.protocol.openai': 'OpenAI / Responses 协议',
        'config.routing.provider.geminiCli': 'Gemini CLI',
        'config.routing.provider.antigravity': 'Antigravity',
        'config.routing.provider.kiro': 'Kiro',
        'config.routing.provider.claudeCompatible': 'Claude 官网 / 反代',
        'config.routing.provider.openaiCompatible': 'OpenAI / Responses / Codex',
        'config.routing.provider.grokCompatible': 'Grok 兼容',
        'config.routing.note.defaultAlias': '包含默认入口',
        'config.routing.note.oneMillionAlias': '包含 1M / -1M 长上下文别名',
        'config.routing.note.directAlias': '支持直接填写真实模型名',
        'config.routing.note.codexDefaultAlias': 'Codex 默认入口',
        'config.routing.note.codexCurrentAlias': 'Codex 当前入口',
        'config.routing.note.preserveClaudeModel': '直连 Claude 上游时保持请求模型；若节点不支持，将由兼容性筛选或 Fallback 接管',
        'config.routing.note.routeToGemini': '统一转成 {model}',
        'config.routing.note.routeToOpenAI': '统一转成 {model}',
        'config.routing.note.routeToGrok': '统一转成 {model}',
        'config.routing.note.routeToKiroHaiku': 'Haiku 保持为 {model}',
        'config.routing.note.routeToKiroSonnet': '除 Haiku 外统一落到 {model}',
        'config.routing.note.routeToAntigravity': '公开模型名 {publicModel}，实际上游请求 {upstreamModel}',
        'config.routing.note.codexToClaudeHigh': '优先映射到 Opus；若节点不支持则回落到 Sonnet',
        'config.routing.note.codexToClaudeStandard': '优先映射到 Sonnet',
        'config.routing.note.openaiPassthrough': '保持 OpenAI/Codex 规范化后的模型名',
        ...
        'providers.account.previewMore': '等 {count} 个账号',
        ...
        'modal.provider.accountLabel': '账号',
        'modal.provider.identityLabel': '账号标识',
        'modal.provider.emailLabel': 'Email',
        'modal.provider.accountIdLabel': 'Account ID',
        'modal.provider.credentialsFileLabel': '凭据文件',
    ...
    'en-US': {
        ...
        'config.sessionAffinity.title': 'Session Affinity & Load Balancing',
        'config.sessionAffinity.enabled': 'Enable Session Affinity',
        'config.sessionAffinity.enabledNote': 'Try to keep the same conversation on the same node to improve cache hit rate and stability',
        'config.sessionAffinity.defaultWeakTtl': 'Weak Session TTL (ms)',
        'config.sessionAffinity.defaultWeakTtlNote': 'Default expiry for weak sessions identified by API Key + IP + UA. Default 1800000 = 30 minutes',
        'config.sessionAffinity.default5xxCoolDown': '5xx Cooldown (ms)',
        'config.sessionAffinity.default5xxCoolDownNote': 'Node cooldown after upstream server errors. Default 60000 = 1 minute',
        'config.sessionAffinity.max429CoolDown': '429 Max Cooldown (ms)',
        'config.sessionAffinity.max429CoolDownNote': 'Maximum exponential backoff cooldown for rate-limit errors. Default 3600000 = 1 hour',
        'config.sessionAffinity.maxSessions': 'Max Cached Sessions',
        'config.sessionAffinity.maxSessionsNote': 'Maximum number of sessions kept in memory before LRU eviction',
        'config.sessionAffinity.virtualNodes': 'Consistent Hash Virtual Nodes',
        'config.sessionAffinity.virtualNodesNote': 'How many virtual nodes each physical node gets. Higher is more even. Default 150',
        ...
        'config.routing.editorLabel': 'Client Model Routing Rules (JSON)',
        'config.routing.editorPlaceholder': 'Manage how Claude / Codex client model names map to real upstream models',
        'config.routing.editorNote': 'This editor stores overrides only. The tables below show the fully effective routing, including Claude / OpenAI protocol aliases, Claude/Codex to Gemini/Claude/OpenAI/Grok upstream routing, and `-1M` style client aliases.',
        'config.routing.previewTitle': 'Client Model Relationship Tables',
        'config.routing.previewNote': 'The JSON editor above manages overrides. The tables below show the effective “client -> provider -> upstream model” results.',
        'config.routing.aliasTableTitle': 'Protocol Alias Normalization',
        'config.routing.aliasTableDescription': 'Lists the accepted downstream model names, 1M/-1M aliases, and canonical model IDs for Claude and OpenAI/Codex protocols.',
        'config.routing.providerTableTitle': 'Provider Upstream Mapping',
        'config.routing.providerTableDescription': 'Shows which upstream model Claude / Codex clients will actually request for each provider family.',
        'config.routing.columns.client': 'Client',
        'config.routing.columns.protocol': 'Protocol',
        'config.routing.columns.canonicalModel': 'Canonical Model',
        'config.routing.columns.aliases': 'Aliases / Inputs',
        'config.routing.columns.requestModel': 'Request Model',
        'config.routing.columns.provider': 'Provider Target',
        'config.routing.columns.upstreamModel': 'Upstream Model',
        'config.routing.columns.note': 'Notes',
        'config.routing.empty': 'No routing rules available.',
        'config.routing.invalidJson': 'Invalid client model routing JSON. Please enter a valid JSON object',
        'config.routing.previewInvalid': 'The current JSON cannot be parsed, so the relationship tables cannot be previewed. Fix it before saving.',
        'config.routing.client.claude': 'Claude Client',
        'config.routing.client.codex': 'Codex Client',
        'config.routing.protocol.claude': 'Claude Protocol',
        'config.routing.protocol.openai': 'OpenAI / Responses Protocol',
        'config.routing.provider.geminiCli': 'Gemini CLI',
        'config.routing.provider.antigravity': 'Antigravity',
        'config.routing.provider.kiro': 'Kiro',
        'config.routing.provider.claudeCompatible': 'Claude Native / Reverse Proxy',
        'config.routing.provider.openaiCompatible': 'OpenAI / Responses / Codex',
        'config.routing.provider.grokCompatible': 'Grok Compatible',
        'config.routing.note.defaultAlias': 'Includes the default entry',
        'config.routing.note.oneMillionAlias': 'Includes 1M / -1M long-context aliases',
        'config.routing.note.directAlias': 'Supports direct real-model IDs',
        'config.routing.note.codexDefaultAlias': 'Codex default entry',
        'config.routing.note.codexCurrentAlias': 'Codex current entry',
        'config.routing.note.preserveClaudeModel': 'When using a Claude upstream directly, keep the requested model; if unsupported, compatibility filtering or fallback handles it',
        'config.routing.note.routeToGemini': 'Unified to {model}',
        'config.routing.note.routeToOpenAI': 'Unified to {model}',
        'config.routing.note.routeToGrok': 'Unified to {model}',
        'config.routing.note.routeToKiroHaiku': 'Haiku stays on {model}',
        'config.routing.note.routeToKiroSonnet': 'Everything except Haiku is routed to {model}',
        'config.routing.note.routeToAntigravity': 'Public model is {publicModel}, actual upstream request is {upstreamModel}',
        'config.routing.note.codexToClaudeHigh': 'Prefers Opus; falls back to Sonnet when Opus is unsupported',
        'config.routing.note.codexToClaudeStandard': 'Prefers Sonnet',
        'config.routing.note.openaiPassthrough': 'Keeps the normalized OpenAI/Codex model name',
        ...
        'providers.account.previewMore': '{count} more accounts',
        ...
        'modal.provider.accountLabel': 'Account',
        'modal.provider.identityLabel': 'Account Identifier',
        'modal.provider.emailLabel': 'Email',
        'modal.provider.accountIdLabel': 'Account ID',
        'modal.provider.credentialsFileLabel': 'Credential File',
